...
function TechTag({ label }: { label: string }) {
  return <span className="inline-flex rounded-full bg-white/[0.06] px-3 py-1.5 text-sm text-text">{label}</span>;
}
        ...
        <article className="section-pad pb-8">
          <div className="mx-auto max-w-4xl">
            ...
            <header className="mt-8">
              <p className="text-xs font-semibold text-accent">{projectsCopy.pageEyebrow}</p>
              <h1 className="mt-3 text-3xl font-semibold tracking-[-0.015em] text-text sm:text-5xl lg:text-6xl">
                {project.name}
              </h1>
              <p className="mt-4 max-w-3xl text-lg leading-relaxed text-text-muted">{project.tagline}</p>

              <div className="mt-6 flex flex-wrap items-center gap-2">
                {project.categories.map((category) => (
                  <span
                    key={category}
                    className="rounded-full border border-line px-3 py-1 tabular-nums text-xs text-text-muted"
                  >
                    {category}
                  </span>
                ))}
                {project.isFinalYearProject && (
                  <span className="rounded-full border border-text-muted/25 bg-text-muted/10 px-3 py-1 text-xs text-text-muted">
              ...
              {(project.status || project.dateRange) && (
                <dl className="mt-6 flex flex-wrap gap-6 text-sm">
                  {project.status && (
                    <div>
                      <dt className="text-xs font-semibold text-text-muted">Status</dt>
                      <dd className="mt-1 text-text-muted">{project.status}</dd>
                    </div>
                  )}
                  {project.dateRange && (
                    <div>
                      <dt className="text-xs font-semibold text-text-muted">Timeline</dt>
            ...
            <div className="mt-12 space-y-10">
              {project.problem && (
                <DetailSection title={projectsCopy.sections.problem}>
                  <p className="text-base leading-relaxed text-text-muted sm:text-[1.05rem]">{project.problem}</p>
                </DetailSection>
              )}

              {project.solution && (
                <DetailSection title={projectsCopy.sections.solution}>
                  <p className="text-base leading-relaxed text-text-muted sm:text-[1.05rem]">{project.solution}</p>
                </DetailSection>
              )}
              ...
              {project.architecture && (
                <DetailSection title={projectsCopy.sections.architecture}>
                  ...
                  {project.architecture.dataFlow && (
                    <p className="mt-4 rounded-2xl bg-bg-elevated p-5 font-mono text-xs leading-relaxed text-text-muted sm:text-sm">
                      {project.architecture.dataFlow}
                    </p>
                  )}
                  {project.architecture.components.length > 0 && (
                    <ul className="mt-6 space-y-3">
                      {project.architecture.components.map((component) => (
                        <li key={component} className="flex gap-3 text-sm leading-relaxed text-text-muted sm:text-base">
                ...
                <DetailSection title={projectsCopy.sections.highlights}>
                  <ul className="space-y-3">
                    {project.highlights.map((highlight) => (
                      <li key={highlight} className="flex gap-3 text-sm leading-relaxed text-text-muted sm:text-base">
                        <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-accent" aria-hidden />
                        <span>{highlight}</span>
                      </li>
                    ))}
                ...
                {techGroups.length > 0 ? (
                  <div className="grid gap-4 sm:grid-cols-2">
                    {techGroups.map(([category, tags]) => (
                      <div key={category} className="tile p-6">
                        <h3 className="mb-3 text-xs font-semibold text-text-muted">{formatTechCategory(category)}</h3>
                ...
                <div className="flex flex-wrap gap-3 border-t border-line pt-10">
                  {live_demo && (
                    <a
                      ...
                      className="inline-flex items-center justify-center press min-h-11 rounded-full bg-button px-6 text-[17px] text-white hover:bg-button-hover"
                    >
            ...
            <nav className="mt-16 grid gap-4 border-t border-line pt-10 sm:grid-cols-2" aria-label="Project navigation">
              {prev ? (
                <Link
